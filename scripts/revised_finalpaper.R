...
evs_clean <- evs_subset %>%
  ...
  mutate(across(c(support_camera, support_email, support_phone), 
         ...
         # pol trust
         pol_trust = case_when(pol_trust %in% 1:4 ~ as.numeric(pol_trust), 
                               pol_trust %in% -10:-1 ~ NA_real_, TRUE ~ NA_real_),
         # soc trust
         soc_trust = case_when(soc_trust %in% 1:2 ~ as.numeric(soc_trust), 
...
# camparison
cat("Number of keeping all denpent variables:", nrow(df_final), "\n")
cat("Number of removing all the missing values:", nrow(df), "\n")
cat("Overall difference:", nrow(df_final) - nrow(df), "(", 
...
# OLS for 3 monitor types seperately (Full)
...
# create a long fomat data
df_long <- df_clean %>% 
  pivot_longer(cols = c(support_camera, support_email, support_phone), 
               names_to = "monitor_type", 
               values_to = "support") %>%
  mutate(monitor_type = factor(monitor_type, 
                               levels = c("support_camera", "support_email", "support_phone"), 
                               labels = c("camera", "email", "phone")),
         
         country_monitor = interaction(country, monitor_type)
  )

# create a interaction model (pol_trust x countries x 3 types of monitor) for H3
full_interaction <- lm(as.numeric(support) ~ pol_trust * monitor_type * country +  
                         age + gender + edu_level + soc_trust,
                       data = df_long)
summary(full_interaction)
stargazer(full_interaction,
          type = "text",
          style = "apsr",
          title = "Public acceptance of 3 Types of Monitor across countries",
          out = "table1.html")

# analyse the slope
slopes <- sim_slopes(full_interaction, 
                     pred = pol_trust, 
                     modx = monitor_type, 
                     mod2 = country)
plot(slopes)
...
# visualizations
h3 <- plot_model(full_interaction, 
                 type = "pred", 
                 terms = c("pol_trust", "monitor_type", "country"),
                 ci.lvl = 0.95,
                 show.data = FALSE) +
  labs(title = "Public Acceptance of 3 Types of Monitor across Countries") +
  theme_sjplot()
h3
